# Flask Pump Monitoring Server – Modern UI + Toggle + Threshold Graphs
...
def load_thresholds():
    ...
    return json.load(open(THRESH_FILE))
...
def thresholds():
    if request.method == "POST":
        ...
        for p in PUMPS:
            t[p] = {
                "warning": float(request.form[f"{p}_w"]),
                "critical": float(request.form[f"{p}_c"])
            }
        json.dump(t, open(THRESH_FILE, "w"), indent=2)
...
@app.route("/")
def home():
    return render_template_string(DASHBOARD_HTML, pumps=PUMPS)
...
TOGGLE_CSS = """
.toggle{width:50px;height:26px;border-radius:20px;background:#ccc;position:relative;cursor:pointer;transition:.3s}
.toggle::after{content:'';width:22px;height:22px;border-radius:50%;background:#fff;position:absolute;top:2px;left:2px;transition:.3s}
...
async function refresh(){{
 ...
 pumps.forEach(p=>{{
   ...
   const th=j.thresholds[p];
   let cls='normal';
   if(t>=th.critical) cls='critical';
   else if(t>=th.warning) cls='warning';
